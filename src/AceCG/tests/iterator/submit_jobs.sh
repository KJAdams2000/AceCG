#!/usr/bin/env bash
# Submit the full FM iterator pipeline: per-window trainer.step() + combine.
...
RUNROOT="${1:-${ACECG_ROOT}/tests/_runs/iterator_$(date +%Y%m%d_%H%M%S)}"
mkdir -p "${RUNROOT}"

echo "[ITERATOR] runroot=${RUNROOT}"
...
WIN_JOB=$(sbatch --parsable --export=ALL "${WINDOWS_SBATCH_ARGS[@]}" \
  "${ACECG_ROOT}/tests/iterator/dopc_windows.sbatch" "${CFG}" "${RUNROOT}")
echo "[ITERATOR] windows job=${WIN_JOB}"

COMB_JOB=$(sbatch --parsable --dependency=afterok:${WIN_JOB} --export=ALL "${COMBINE_SBATCH_ARGS[@]}" \
  "${ACECG_ROOT}/tests/iterator/dopc_combine.sbatch" "${CFG}" "${RUNROOT}")
echo "[ITERATOR] combine job=${COMB_JOB}"
...
echo "[ITERATOR] metadata written to ${RUNROOT}/metadata.json"
echo "[ITERATOR] ACECG_ROOT=${ACECG_ROOT}"
echo "[ITERATOR] ACECG_PYTHON=${ACECG_PYTHON}"
echo "[ITERATOR] Monitor: squeue -u \$USER ; sacct -j ${WIN_JOB},${COMB_JOB} --format=JobID,State,Elapsed"
